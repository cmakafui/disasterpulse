...
function Disaster({ disasters, onHoverDisaster }: Props) {
  ...
  const handleViewInsights = (id: number) => {
    router.push(`/disaster/${id}`);
  };
  ...
  return (
    <div className="space-y-6">
      {disasters.map((disaster) => (
        <Card
          key={disaster.id}
          className="w-full hover:shadow-lg transition-all duration-300 cursor-pointer rounded-lg border-2 border-gray-100 hover:border-primary overflow-hidden mb-6"
        ...
        >
          ...
          <CardFooter className="bg-gray-50 flex flex-col sm:flex-row justify-between items-center p-4 space-y-2 sm:space-y-0">
            ...
            <Button
              ...
              size="sm"
              className="ml-4"
              onClick={() => handleViewInsights(disaster.id)}
            >
              View Insights
